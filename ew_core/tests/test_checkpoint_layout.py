...
class CanonicalLayoutTests(unittest.TestCase):
    ...
    def test_trainers_use_resolver(self):
        from ew_core.training import train_deinterleaver, train_scheduler
    ...
    def test_export_onnx_writes_onnx_metadata(self):
        src = (ROOT / "ew_core" / "deployment" / "export_onnx.py").read_text(encoding="utf-8")
...
class TrainingConfigTests(unittest.TestCase):
    ...
    def test_checkpoint_references_are_canonical(self):
        self.assertIn(
            self.cfg.get("deinterleaver_ckpt"),
            ("checkpoints/deinterleaver/best.pt", "experiments/checkpoints/deinterleaver/best.pt"),
        )
        self.assertIn(
            self.cfg.get("normalization_stats"),
            ("checkpoints/deinterleaver/normalization_stats.json", "experiments/checkpoints/deinterleaver/normalization_stats.json"),
        )
