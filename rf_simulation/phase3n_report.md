...
### 4. RF change summary
`d3eca70..2012fa2`: 31 tracked files + `.gitattributes`, entirely under `rf_simulation/`.
...
`emitter_id` hits in rf_simulation are **comments/documentation only** (dwell_orchestrator.py L13/L150, frequency_context.py L22) — the code explicitly states "no ground truth is used." `ground_truth_pdws` in test_iq_to_pdw.py is the test's own local ground truth, not scheduler data. **No leakage.**
...
Zero machine-specific runtime Python paths in rf_simulation scripts/tests.
