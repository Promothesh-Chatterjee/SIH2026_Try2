# Phase 3E — Master + rf_simulation Integration Audit
...
└── SIH 2026\               <- worktree holding the SECOND repo (rf_simulation)
    ├── .git\
    └── rf_simulation\         <- GNU Radio repo (branch: main, HEAD e55d2cf, 6 tracked files)
...
Note: The GNU Radio repo lives at `C:\HACKATHONS\SIH 2026\rf_simulation` — i.e. it is nested *inside the workspace root folder* `C:\HACKATHONS\SIH 2026`, whose `.git` is at `C:\HACKATHONS\SIH 2026\.git`.

## 2. Exact Location of All Phase 3A–3D Files (verified present)

| File | Size (bytes) |
|------|-------------|
| `rf_simulation\scripts\iq_to_pdw.py` | 14492 |
| `rf_simulation\scripts\frequency_context.py` | 6025 |
| `rf_simulation\scripts\iq_bridge.py` | 5863 |
| `rf_simulation\scripts\dwell_orchestrator.py` | 7482 |
| `rf_simulation\tests\test_iq_to_pdw.py` | 19433 |
| `rf_simulation\tests\test_frequency_context.py` | 6619 |
| `rf_simulation\tests\test_iq_bridge.py` | 13385 |
| `rf_simulation\tests\test_dwell_orchestrator.py` | 16982 |
| `rf_simulation\scripts\proof_phase3c.py` | 1553 |
| `rf_simulation\scripts\proof_phase3d.py` | 2480 |

Authoritative master receiver verified at `SIH2026_Try2\ew_core\src\receiver\`:
...
| `dwell_orchestrator.py` | **YES** | `sys.path.insert(0, r"C:\HACKATHONS\SIH2026_Try2\ew_core\src")` then `from receiver import SieveReceiver`. **Hardcoded absolute Windows path.** |
| `test_frequency_context.py` | **YES** | hardcoded `_MASTER_SRC = r"C:\HACKATHONS\SIH2026_Try2\ew_core\src"` |
...
- The **only** cross-repo dependency is on the authoritative `SieveReceiver`, reached exclusively via a **hardcoded absolute Windows path** (`C:\HACKATHONS\SIH2026_Try2\ew_core\src`). This is the packaging defect.
- Mechanism is CWD-independent (verified: tests + proofs run from both rf_simulation and SIH2026_Try2 CWDs, and with `PYTHONPATH` cleared).
...
## 4. rf_simulation Test Results (RadioConda)
...
**rf_simulation (`SIH 2026\rf_simulation`):**
```
 M README.md
...
?? tests/       (Phase 3A–3D)
```
No tracked file in rf_simulation was modified during this phase. All Phase 3 scripts/tests remain untracked (never committed).

## 7. Explanation of the ZIP Omission of rf_simulation

Two submission archives were found:
- `C:\HACKATHONS\SIH 2026.zip` (837MB, dated 2026-08-29) — contains only `SIH 2026/.git` + worktree; **no rf_simulation at all** (predates it).
- `C:\HACKATHONS\SIH2026_Try2.zip` (60MB, dated 2026-09-04) — contains `SIH2026_Try2/` with `ew_core/` **and the `.git/` directory inside**, and **NO `rf_simulation/`**.

**Root cause:** rf_simulation is a *separate git repository* living in a different worktree (`C:\HACKATHONS\SIH 2026\rf_simulation`), which is a **sibling** of `SIH2026_Try2` at the filesystem level — not a child. The submission ZIP was created from `SIH2026_Try2` alone, so rf_simulation was never included. The only "PDW" matches in the ZIP are `frontend/src/components/PDWFeed.jsx` and `PDWScatter.jsx` (React UI), **not** the RF pipeline.
...
├── ew_core/       <- existing application (unchanged, authoritative receiver)
└── rf_simulation/                    <- moved IN as a top-level directory
    ├── scripts/  tests/  flowgraphs/  recordings/  README.md
```

**Rationale (based on actual code, not aesthetics):**
- The extreme coupling: `dwell_orchestrator.py` hardcodes `C:\HACKATHONS\SIH2026_Try2\ew_core\src`. Under Option A, this becomes `relative` or a clean `PYTHONPATH` from a single repo root — eliminating the only cross-repo path dependency.
...
- Option B puts GNU Radio *inside* `ew_core\gnuradio\`, which blends two unrelated subsystems into one Python package namespace and complicates the app's packaging (`pyproject.toml`), for no benefit. Rejected.
...
**Important caveat for Option A:** rf_simulation depends on **RadioConda** python (for numpy) and real GNU Radio for live flowgraphs, while the master app uses its own venv. Option A keeps rf_simulation as a self-contained directory that is still run with RadioConda — it does not force GNU Radio into the master venv. The master ML system can still be deployed/run **without** GNU Radio installed (GNU Radio is only needed for the RF-simulation half). Duplication is avoided: the receiver is never copied; rf_simulation imports it from the single root.

## 10. Recommended Submission Structure

Ship **one** root folder (Option A) with the rf_simulation merged in, zipped without the `.git` directories:
```
SIH2026_Try2_Submission/
├── README.md                        (explains master vs RF, how to run each)
├── ew_core/         (MASTER: app; keep its .git OUT of submission or a single clean .git)
└── rf_simulation/                      (RF pipeline + flowgraphs)
```
- **A (required to run master ML):** `ew_core`, `tests`, `configs`, `scripts`, `pyproject.toml`, `requirements.txt`, `README.md`, `Architecture.md`, `Design.md`, `PRD.md`, `QuickStart`, `.env.example`.
- **B (required to run RF simulation):** `rf_simulation/scripts` (Phase 3A–3D), `rf_simulation/tests`, `rf_simulation/flowgraphs`, `rf_simulation/README.md`, plus a note that RadioConda + GNU Radio is required.
...
| `__pycache__/` (10 dirs) + `.pyc` (51 files) | master src/tests, rf_simulation | EXCLUDE |
| `.pytest_cache/` | master | EXCLUDE |
| `checkpoints/*.pt` (6 files, ~19MB, tracked) | master | REVIEW (see below) |
| `recordings/*.dat` (5×16MB IQ) | rf_simulation | EXCLUDE (already gitignored) |
...
| flowgraphs/*.grc */**.py | rf_simulation | KEEP (required for RF) |
...
1. **[Packaging] Hardcoded absolute imports to the master receiver.** `dwell_orchestrator.py` (L36) and the two bridge/dwell test files insert `r"C:\HACKATHONS\SIH2026_Try2\ew_core\src"` directly. This is the single reproducibility blocker — it only works on this machine at this path. It is NOT a master-repo defect; a fix belongs in the rf_simulation scripts/tests (make the path derived from a repo-relative or env-driven root). This is a rf_simulation-side packaging defect, safe to fix in the RF-side code — **no master source modification needed.**
2. **[Submission] rf_simulation omitted from the archive** (Section 7) — the RF pipeline is not demonstrable from the current ZIP.
...
**No.** The master repository required **zero** source changes (still clean, HEAD `2b4333b`, 46/46 tests). No rf_simulation tracked file was modified during this audit. Only new untracked Phase 3 files already existed; nothing was deleted, reset, or rewritten. Per instructions, no line-ending rewrite and no git reset were performed.
...
1. **Single-root merge (Option A):** copy/move `C:\HACKATHONS\SIH 2026\rf_simulation` into `SIH2026_Try2\rf_simulation` so master and RF share one repo root.
2. **Remove the hardcoded absolute path** in the GNU RF scripts/tests: replace the `sys.path.insert(0, r"C:\...\SIH2026_Try2\ew_core\src")` with a **repo-relative** resolution (e.g., `Path(__file__).resolve().parents[2] / "ew_core"/ "src"`) so it works from any checkout location.
...
4. **Fix the submission:** produce a single clean ZIP (no `.git/`, no caches, no recordings; exclude or keep checkpoints per decision) that contains both `ew_core` and `rf_simulation`, and add a README recording the RadioConda (RF) vs venv (master) interpreters.
