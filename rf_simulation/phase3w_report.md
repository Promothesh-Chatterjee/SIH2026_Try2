...
1. `rf_simulation/scripts/dwell_orchestrator.py` — `run_generated_dwell` default `detector_threshold_db` 5.0 → 15.0
   (+ docstring).
2. `rf_simulation/scripts/rf_env_adapter.py` — `RFEnvAdapter.__init__` default `detector_threshold_db` 5.0 → 15.0
   (+ docstring).
3. `rf_simulation/scripts/dwell_orchestrator.py` — `run_generated_dwell` now filters emitters to the receiver's IBW
   window `[center − ibw/2, center + ibw/2]` before IQ generation, so out-of-band emitters are physically invisible.
4. `rf_simulation/scripts/rf_env_adapter.py` — `RFEnvAdapter.step` applies the identical IBW emitter filter in its
   `center` tune.
...
`rf_simulation/tests/test_generated_dwell.py` — new classes:
