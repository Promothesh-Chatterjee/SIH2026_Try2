...
└── SIH 2026\                 <- SECOND git repo (branch main, HEAD e55d2cf)
    ├── .git\
    └── rf_simulation\           <- RF subsystem (tracked 6 files by the SIH 2026 repo)
```
...
├── .gitignore
├── ew_core\    <- MASTER app (unchanged, authoritative receiver)
└── rf_simulation\                 <- MOVED-IN RF subsystem (now a plain dir)
```

## 2. Exact Files Moved

The entire `C:\HACKATHONS\SIH 2026\rf_simulation` tree was copied to `C:\HACKATHONS\SIH2026_Try2\rf_simulation` (copy chosen so the original `SIH 2026` worktree stays intact as a safety net). Contents moved (all verified present at the new root):
...
Original source at `C:\HACKATHONS\SIH 2026\rf_simulation` left **intact**. No destructive removal anywhere.

## 3. Exact Files Modified

| File (new location) | Change |
|---------------------|--------|
| `rf_simulation\scripts\dwell_orchestrator.py` | master path → repo-relative (`parents[2]`) |
| `rf_simulation\scripts\proof_phase3c.py` | both abs paths → repo-relative |
| `rf_simulation\scripts\proof_phase3d.py` | both abs paths → repo-relative |
| `rf_simulation\scripts\jittered_rf_source.py` | file-sink path → repo-relative `recordings/` |
| `rf_simulation\tests\test_frequency_context.py` | `_MASTER_SRC` → repo-relative |
| `rf_simulation\tests\test_iq_bridge.py` | `_MASTER_SRC` → repo-relative |
| `rf_simulation\tests\test_dwell_orchestrator.py` | master path → repo-relative |
| `rf_simulation\README.md` | layout/environments/integration docs updated |
| `SIH2026_Try2\.gitattributes` | NEW (minimal LF normalization policy) |
| `rf_simulation\tests\test_repo_integration.py` | NEW (cross-repo integration tests) |

**Master production source: NONE modified.** `git diff -- ew_core/receiver/` is empty.

## 4. Exact Hardcoded Paths Removed

All of these were removed from runtime source:
- `r"C:\HACKATHONS\SIH2026_Try2\ew_core\src"` — in dwell_orchestrator.py, proof_phase3c.py, proof_phase3d.py, test_dwell_orchestrator.py
- `_MASTER_SRC = r"C:\HACKATHONS\SIH2026_Try2\ew_core\src"` — in test_frequency_context.py, test_iq_bridge.py
- `r"C:\HACKATHONS\SIH 2026\rf_simulation\scripts"` — in proof_phase3c.py, proof_phase3d.py
- `"C:\\HACKATHONS\\SIH 2026\\rf_simulation" ... "recordings\..."` — file sink in jittered_rf_source.py
...
```python
# In <repo_root>/rf_simulation/scripts/*.py  and  <repo_root>/rf_simulation/tests/*.py
...
```
- `parents[2]` = `<repo_root>` for both `rf_simulation/scripts/**` and `rf_simulation/tests/**`.
- Local GNU RF scripts are resolved via `Path(__file__).resolve().parent` (scripts dir).

## 6. Did rf_simulation Become Tracked by the Master Repo?
...
?? rf_simulation/
```
It is **NOT** a submodule (no `.git`/`.gitmodules`; `git submodule status` empty; `git ls-files` shows 0 rf_simulation entries). It is ready to be `git add`ed by the user when they review.

## 7. Was Any Git History Preserved/Changed?

**Nothing destroyed.** Critical finding: `rf_simulation` was **never its own git repository** — its git history (`e55d2cf`, etc.) belongs to the parent `C:\HACKATHONS\SIH 2026` repo (which contains rf_simulation plus many unrelated dirs). Therefore no submodule, no history merge, and no destructive rewrite was needed. The `SIH 2026` repo and its full history remain untouched in place. A full backup of `rf_simulation` (including its git connection) was also preserved at the source location and the move was a copy.
...
```
C:\Users\Indrani\radioconda\python.exe -m unittest discover -s rf_simulation\tests
...
`rf_simulation/tests/test_repo_integration.py` (6 tests, all pass) proves:
...
- CWD = `rf_simulation` → 97, then 103 OK
...
git diff -- ew_core/receiver/: (empty — receiver untouched)
git submodule status: (empty — rf_simulation is NOT a submodule)
```
...
2. **Master regression must be run from `ew_core`** (its rootdir puts `src/` on the path). This is the existing, documented master convention — unchanged, not a defect.
...
4. **rf_simulation is not yet committed** — it is staged only as untracked; a human review + `git add` is the next deliberate step.
...
1. **Review and commit the Phase 3F integration** in the master repo: `git add rf_simulation/ .gitattributes` then a human-reviewed commit. This makes the single root a committed, trackable, reproducible unit and fixes the original ZIP omission.
2. **Add a top-level `README.md` / "How to run"** explaining the two environments (master venv vs RadioConda) using the existing rf_simulation README content.
