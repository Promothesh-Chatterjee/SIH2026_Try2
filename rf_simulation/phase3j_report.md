...
```
ew_core/environment/cognitive_rf_scan_env.py  (modified by d3eca70)
   └─ from ew_core.cognitive.memory import SemanticMemory, EmitterProfile   → src/cognitive/memory.py:15 `import torch`
   └─ from ew_core.cognitive.periodic_interceptor import PeriodicScanInterceptor → src/cognitive/periodic_interceptor.py:12 `from scipy.signal import find_peaks`
```
...
Changes not staged for commit:
    modified:  experiments/checkpoints/best.pt   (Bin 3259969 -> 3259969 bytes)
    modified:  experiments/checkpoints/final.pt  (Bin 3259391 -> 3259391 bytes)
Untracked (docs): rf_simulation/phase3g_report.md, phase3h_report.md, phase3i_report.md
```
...
2. **Decide the two checkpoint `.pt` files** (they were inadvertently modified by test execution): either restore them (`git checkout -- experiments/checkpoints/best.pt final.pt`) or accept/re-commit as the owner prefers — I did not touch them.
