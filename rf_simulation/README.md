# rf_simulation — GNU Radio RF Environment
...
`SIH2026_Try2/rf_simulation`. It sits alongside (NOT inside) the master
application `SIH2026_Try2/ew_core`. The two share one
...
```
SIH2026_Try2/
├── ew_core/       <- MASTER application (receiver, perception, scheduler)
└── rf_simulation/                    <- RF simulation + PDW adapter (THIS subsystem)
```

## Python Environments

- **MASTER** (`ew_core`): runs in its own venv / project
  environment (see `ew_core/requirements.txt`).
- **RF** (`rf_simulation`): requires **RadioConda + GNU Radio** (see below).
...
SOURCE A: Existing structured RF world
    ...
    ▼
    Existing SieveReceiver (ew_core/receiver/)
...
`rf_simulation` does NOT contain a duplicate receiver. It consumes the
authoritative `SieveReceiver` from `ew_core/receiver/`
through a checkout-relative integration path
(`repo_root/ew_core`). There must never be a second
