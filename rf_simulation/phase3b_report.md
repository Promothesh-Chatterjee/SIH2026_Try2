...
- **Date:** 2026-09-04
- **Repos:** `SIH2026_Try2` (master, untouched) and `SIH 2026\rf_simulation` (new code)
...
| File | Purpose |
|------|---------|
| `rf_simulation/scripts/frequency_context.py` | Frequency-context bridge (pure, no GNU Radio / no receiver duplicate) |
| `rf_simulation/tests/test_frequency_context.py` | 20 unit tests (A–G matrix) |

## 2. Files Changed

| File | Change |
|------|--------|
| `rf_simulation/scripts/frequency_context.py` | (new, see above) |
| `rf_simulation/tests/test_frequency_context.py` | (new, see above) |
...
- Combined rf_simulation suite: **40** passing

## 12. Receiver Regression Results

Run from `SIH2026_Try2\ew_core`:
...
Phase 3C (integration shim) should introduce, in **`rf_simulation`** (with only a thin,
...
**Create (in rf_simulation):**
- `rf_simulation/scripts/iq_bridge.py` — per-dwell adapter: consumes PDW NDJSON, holds a
  `FrequencyContext`, emits receiver-compatible pulse records, feeds the receiver.
- `rf_simulation/tests/test_iq_bridge.py` — bridge tests (mapping, tuning-context, window, isolation).

**Change (rf_simulation, after explicit scoping):**
- `rf_simulation/scripts/live_rf_environment.py` — parameterize LO/center so one emitter is
  observable per tune (minimal change; DO NOT redesign unless required).
- `rf_simulation/scripts/iq_to_pdw.py` — add optional carrier/LO annotation (context metadata on
