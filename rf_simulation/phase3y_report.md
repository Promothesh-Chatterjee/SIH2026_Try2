...
`rf_simulation/DATASET_SPEC.md`, schema **`3Y.1`**:
...
`rf_simulation/scripts/dataset_generator.py` (~870 lines). Reuses only existing
...
`rf_simulation/tests/test_dataset_generator.py` — **13 tests** covering the 10
required checks:
...
## 20. Regression suite C — master non-training (PYTHONPATH=ew_core)
...
- Phase 3Y modified no master files (git status shows changes only in rf_simulation).
...
`git status --short` shows: `M rf_simulation/scripts/dwell_orchestrator.py`
(Phase 3W fix, unchanged this phase), pre-existing `M best.pt`/`M final.pt`
(working-tree dirty, timestamps untouched), and untracked rf_simulation Phase 3X
deliverables. **No staging, committing, or pushing performed.** No master
source file was modified.

## 23. Dry-run corpus (Steps 16, 23)

`rf_simulation/dry_run_corpus` — **4 episodes × 20 dwells = 80 observation rows**,
