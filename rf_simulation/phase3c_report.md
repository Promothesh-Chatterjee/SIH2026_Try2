...
| File | Purpose |
|------|---------|
| `rf_simulation/scripts/iq_bridge.py` | IQReceiverBridge: PDW→pulse conversion + SieveReceiver integration |
| `rf_simulation/tests/test_iq_bridge.py` | 25 unit tests (A–L matrix) |
| `rf_simulation/scripts/proof_phase3c.py` | Manual end-to-end proof script |
...
```python
sys.path.insert(0, r"C:\HACKATHONS\SIH2026_Try2\ew_core\src")
...
- `rf_simulation/scripts/dwell_orchestrator.py` — per-tune sweep controller
- `rf_simulation/tests/test_dwell_orchestrator.py`

Exact files Phase 3D would minimally modify:
- `rf_simulation/scripts/live_rf_environment.py` — per-tune baseband LO parameterization
