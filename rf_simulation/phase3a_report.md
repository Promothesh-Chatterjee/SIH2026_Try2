...
- **Date:** 2026-09-04
- **Repos involved:** `SIH2026_Try2` (master, untouched) and `SIH 2026\rf_simulation` (new code)
...
| File | Purpose |
|------|---------|
| `rf_simulation/scripts/iq_to_pdw.py` | IQ→PDW extractor: `PDWDetector` (pure numpy) + ZMQ live reader + raw-file reader + CLI |
| `rf_simulation/tests/test_iq_to_pdw.py` | Unit tests for the detector core (20 tests) |
| `rf_simulation/.gitignore` | (Phase 2) ignores `__pycache__/`, `*.pyc`, `recordings/*.dat` |
...
Run them with:
```
C:\Users\Indrani\radioconda\python.exe -m unittest discover -s "C:\HACKATHONS\SIH 2026\rf_simulation\tests"
```
...
references), `rf_simulation/README.md` documents architecture, `recordings/*.dat` gitignored.
...
& "C:\Users\Indrani\radioconda\python.exe" -m unittest discover -s `
  "C:\HACKATHONS\SIH 2026\rf_simulation\tests"

# 2. Live end-to-end
& "C:\Users\Indrani\radioconda\python.exe" `
  "C:\HACKATHONS\SIH 2026\rf_simulation\scripts\live_rf_environment.py"
# (in a second window)
& "C:\Users\Indrani\radioconda\python.exe" `
  "C:\HACKATHONS\SIH 2026\rf_simulation\scripts\iq_to_pdw.py" --duration 6 --threshold-db 8
```
...
- Extractor: `rf_simulation\scripts\iq_to_pdw.py`
- Tests: `rf_simulation\tests\test_iq_to_pdw.py`
- Live source (existing): `rf_simulation\scripts\live_rf_environment.py`
