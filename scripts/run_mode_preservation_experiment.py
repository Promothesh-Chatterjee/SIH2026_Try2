...
from ew_core.training.diagnostics.mode_telemetry import compute_mode_isolated_gradients
from scripts.compare_identical_state_traces import collect_identical_state_trace
...
def main():
    device_str = "cpu"
    base_ckpt_path = Path("experiments/checkpoints/production_baseline/checkpoint_gate_25000_frozen.pt")
    base_ckpt = torch.load(base_ckpt_path, map_location=device_str, weights_only=False)
    base_state_dict = base_ckpt["state_dict"]

    res_path = Path("experiments/checkpoints/production_baseline/baseline_reservoir_5k.pkl")
    ...
    # Save final report
    out_path = Path("experiments/reports/mode_preservation_experiment_report.json")
    ...
    traces_path = Path("experiments/reports/identical_state_traces_sample.json")
