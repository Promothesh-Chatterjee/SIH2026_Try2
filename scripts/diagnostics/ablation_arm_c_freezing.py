"""Phase 9C - Arm C: Representation Freezing Remediation Ablation.
...
logger = logging.getLogger("ablation_arm_c")

REPO_ROOT = Path(__file__).resolve().parents[3]
PKG_ROOT = REPO_ROOT / "ew_core"
...
def train_c_variant(
    ...
    logger.info("Training %s: train_value=%s, train_aux=%s (%d steps)", variant_name, train_value, train_aux, steps)
    ...
    if train_aux:
        for p in model.intercept_prob_head.parameters():
            p.requires_grad = True
            trainable_params.append(p)
        for p in model.intercept_time_head.parameters():
            p.requires_grad = True
            trainable_params.append(p)

    if not trainable_params:
        logger.info("%s has zero trainable parameters (pure inference probe). Returning untouched model.", variant_name)
        return model
...
def run_arm_c() -> Dict[str, Any]:
    logger.info("Executing Phase 9C - Arm C: Representation Freezing Remediation Ablation...")
    ...
    # Variant C1: Value Stream only
    m_c1 = train_c_variant("Variant C1 (Value Stream only)", True, False, parent_path, reservoir_path, steps=250)
    eval_c1 = evaluate_model_multi_seed(m_c1, seeds, n_steps=eval_steps)

    # Variant C2: Auxiliary Heads only
    m_c2 = train_c_variant("Variant C2 (Auxiliary Heads only)", False, True, parent_path, reservoir_path, steps=250)
    eval_c2 = evaluate_model_multi_seed(m_c2, seeds, n_steps=eval_steps)

    # Variant C3: Value Stream + Auxiliary Heads
    m_c3 = train_c_variant("Variant C3 (Value + Aux)", True, True, parent_path, reservoir_path, steps=250)
    eval_c3 = evaluate_model_multi_seed(m_c3, seeds, n_steps=eval_steps)

    # Variant C4: Pure Inference Stability Probe (all parameters frozen)
    m_c4 = train_c_variant("Variant C4 (Pure Inference Stability)", False, False, parent_path, reservoir_path, steps=0)
    eval_c4 = evaluate_model_multi_seed(m_c4, seeds, n_steps=eval_steps)

    # Save Checkpoint Artifacts
    out_dir = PKG_ROOT / "checkpoints" / "phase9c_remediation" / "ablation_c"
    out_dir.mkdir(parents=True, exist_ok=True)
    torch.save({"state_dict": m_c1.state_dict(), "variant": "C1"}, out_dir / "candidate_c1_value_only.pt")
    torch.save({"state_dict": m_c2.state_dict(), "variant": "C2"}, out_dir / "candidate_c2_aux_only.pt")
    torch.save({"state_dict": m_c3.state_dict(), "variant": "C3"}, out_dir / "candidate_c3_value_plus_aux.pt")
    torch.save({"state_dict": m_c4.state_dict(), "variant": "C4"}, out_dir / "candidate_c4_pure_stability.pt")

    results_payload = {
        "experiment": "phase9c_arm_c_representation_freezing",
        ...
        "multi_seed_evaluations": {
            "parent_champion_reference": eval_c4,
            "variant_c1_value_only": eval_c1,
            "variant_c2_aux_only": eval_c2,
            "variant_c3_value_plus_aux": eval_c3,
            "variant_c4_pure_stability": eval_c4,
        },
        "safety_assertions": {
    ...
    report_path = PKG_ROOT / "reports" / "diagnostics" / "phase9c_ablation_arm_c_results.json"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    with open(report_path, "w", encoding="utf-8") as f:
        json.dump(results_payload, f, indent=2)

    logger.info("Arm C finished successfully. Report saved to: %s", report_path)
    return results_payload


if __name__ == "__main__":
    res = run_arm_c()
    for v_key in ["variant_c1_value_only", "variant_c2_aux_only", "variant_c3_value_plus_aux", "variant_c4_pure_stability"]:
